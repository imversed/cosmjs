...
import {
  ...
  toHex,
  toUtf8,
} from "@imversed/encoding";
import { assert, isNonNullObject } from "@imversed/utils";
...
export class Secp256k1HdWallet implements OfflineAminoSigner {
  ...
  public async signAmino(
    ...
    switch (urlType) {
      case "/ethermint.crypto.v1.ethsecp256k1.PubKey": {
        // eth signing
        const hashedMessage = new Keccak256(serializeSignDoc(signDoc, "Ethermint")).digest();
      ...
      default: {
        // cosmos sigining
        const hashedMessage = sha256(serializeSignDoc(signDoc));
        const signature = await Secp256k1.createSignature(hashedMessage, privkey);
        const signatureBytes = new Uint8Array([...signature.r(32), ...signature.s(32)]);
  ...
  private static toChecksummedAddress(address: string): string {
    ...
    if (typeof address === "string") {
      if (!Secp256k1HdWallet.isValidAddress(address)) {
  ...
  private static async getBech32AddressFromEVMAddress(
    ...
    if (!Secp256k1HdWallet.isAddress(evmAddress.toLowerCase())) {
      ...
      this.accounts.map(async ({ hdPath, prefix }) => {
        ...
        switch (coinType) {
          case "60'":
            ...
            // EVM address
            const address = Secp256k1HdWallet.toChecksummedAddress("0x" + lastTwentyBytes);

            return {
              algo: "secp256k1" as const,
              privkey: privkey,
              pubkey: Secp256k1.compressPubkey(pubkey),
              address: await Secp256k1HdWallet.getBech32AddressFromEVMAddress(address, prefix),
            };
          default:
            return {
              algo: "secp256k1" as const,
